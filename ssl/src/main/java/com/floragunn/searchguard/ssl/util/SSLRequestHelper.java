...
public class SSLRequestHelper {
    ...
    public static class SSLInfo {
        ...
        public String toString() {
            return "SSLInfo [x509Certs=" + Arrays.toString(x509Certs) + ", principal=" + principal + ", protocol=" + protocol + ", cipher=" + cipher
                    + "]";
        }

    }

    public static SSLInfo getSSLInfo(final Settings settings, final Path configPath, final RestRequest request, PrincipalExtractor principalExtractor)
    ...
    private static boolean validate(X509Certificate[] x509Certs, final Settings settings, final Path configPath) {
        ...
        try {
            ...
            if (truststore != null) {
                ...
                final File trustedCas = env.configFile().resolve(settings.get(SSLConfigConstants.SEARCHGUARD_SSL_HTTP_PEMTRUSTEDCAS_FILEPATH, ""))
