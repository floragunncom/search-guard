...
public class InitializationIntegrationTests extends SingleClusterTest {
    ...
    public void testEnsureInitViaRestDoesWork() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_SERVICE_UNAVAILABLE,
                rh.executePutRequest("searchguard/_doc/0", "{}", encodeBasicHeader("___", "")).getStatusCode());

        rh.keystore = "kirk-keystore.jks";
        Assert.assertEquals(HttpStatus.SC_CREATED,
                rh.executePutRequest("searchguard/_doc/config", "{}", encodeBasicHeader("___", "")).getStatusCode());
    ...
    public void testConfigHotReload() throws Exception {
        ...
        for (Iterator<TransportAddress> iterator = clusterInfo.httpAdresses.iterator(); iterator.hasNext();) {
            ...
            HttpResponse res = rh.executeRequest(
                    new HttpGet(
                            "http://" + TransportAddress.getAddress() + ":" + TransportAddress.getPort() + "/" + "_searchguard/authinfo?pretty=true"),
                    spock);
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            ...
            tc.index(new IndexRequest("searchguard").setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("internalusers").source("internalusers",
                    FileHelper.readYamlContent("sg_internal_users_spock_add_roles.yml"))).actionGet();
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            Assert.assertEquals(clusterInfo.numNodes, tc.admin().cluster().nodesInfo(new NodesInfoRequest()).actionGet().getNodes().size());
            tc.index(new IndexRequest("searchguard").setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("config").source("config",
                    FileHelper.readYamlContent("sg_config_anon.yml"))).actionGet();
        ...
        for (Iterator<TransportAddress> iterator = clusterInfo.httpAdresses.iterator(); iterator.hasNext();) {
            TransportAddress TransportAddress = (TransportAddress) iterator.next();
            HttpResponse res = rh.executeRequest(new HttpGet(
                    "http://" + TransportAddress.getAddress() + ":" + TransportAddress.getPort() + "/" + "_searchguard/authinfo?pretty=true"));
    ...
    public void testDefaultConfig() throws Exception {
        final Settings settings = Settings.builder().put("searchguard.allow_default_init_sgindex", true).build();
        setup(Settings.EMPTY, null, settings, false);
        RestHelper rh = nonSslRestHelper();
    ...
    public void testDiscoveryWithoutInitialization() throws Exception {
        ...
        Assert.assertEquals(clusterInfo.numNodes,
                clusterHelper.nodeClient().admin().cluster().health(new ClusterHealthRequest().waitForGreenStatus()).actionGet().getNumberOfNodes());
        Assert.assertEquals(ClusterHealthStatus.GREEN,
