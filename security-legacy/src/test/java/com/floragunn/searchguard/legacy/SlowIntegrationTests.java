...
public class SlowIntegrationTests extends SingleClusterTest {
    ...
    public void testCustomInterclusterRequestEvaluator() throws Exception {
        ...
        final Settings settings = Settings.builder()
                ...
                .put("discovery.initial_state_timeout", "8s").build();
    ...
    public void testNodeClientAllowedWithServerCertificate() throws Exception {
        ...
        final Settings tcSettings = Settings.builder().put(minimumSearchGuardSettings(Settings.EMPTY).get(0))
                .put("cluster.name", clusterInfo.clustername).put("node.data", false).put("node.master", false).put("node.ingest", false)
                .put("path.home", "/tmp").put("node.name", "transportclient").put("discovery.initial_state_timeout", "8s")
                .putList("discovery.zen.ping.unicast.hosts", clusterInfo.nodeHost + ":" + clusterInfo.nodePort).build();
        ...
        try (Node node = new PluginAwareNode(false, tcSettings, Netty4Plugin.class, SearchGuardPlugin.class).start()) {
            Assert.assertFalse(node.client().admin().cluster()
                    .health(new ClusterHealthRequest().waitForNodes(String.valueOf(clusterInfo.numNodes + 1))).actionGet().isTimedOut());
            Assert.assertEquals(clusterInfo.numNodes + 1,
    ...
    public void testNodeClientDisallowedWithNonServerCertificate() throws Exception {
        ...
        final Settings tcSettings = Settings.builder().put(minimumSearchGuardSettings(Settings.EMPTY).get(0))
                ...
                .put(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_ALIAS, "kirk").build();
