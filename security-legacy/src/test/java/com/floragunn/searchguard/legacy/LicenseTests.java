...
public class LicenseTests extends SingleClusterTest {
    ...
    @Ignore // TODO test invalid license signature when uploading new license
    @Test
    ...
    public void testFullLicense() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("_searchguard/license?pretty", encodeBasicHeader("nagilum", "nagilum"))).getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testComplianceLicenseMap() throws Exception {
        ...
        SearchGuardLicense license = new SearchGuardLicense(
                XContentHelper.convertToMap(new BytesArray(FileHelper.loadFile("license1.json")), false, JsonXContent.jsonXContent.type()).v2());

        Assert.assertFalse(license.hasFeature(Feature.COMPLIANCE));
        Assert.assertArrayEquals(license.getFeatures(), new Feature[0]);

        license = new SearchGuardLicense(
                XContentHelper.convertToMap(new BytesArray(FileHelper.loadFile("license3.json")), false, JsonXContent.jsonXContent.type()).v2());

        Assert.assertFalse(license.hasFeature(Feature.COMPLIANCE));
        Assert.assertArrayEquals(license.getFeatures(), new Feature[0]);

        license = new SearchGuardLicense(
                XContentHelper.convertToMap(new BytesArray(FileHelper.loadFile("license2.json")), false, JsonXContent.jsonXContent.type()).v2());
