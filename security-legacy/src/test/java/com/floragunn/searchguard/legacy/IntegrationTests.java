...
public class IntegrationTests extends SingleClusterTest {
    ...
    public void testDnParsingCertAuth() throws Exception {
        ...
        Assert.assertArrayEquals(new String[] { "ert" },
                auth.extractCredentials(null, newThreadContext("cn=abc,l=ert,st=zui,c=qwe")).getBackendRoles().toArray(new String[0]));
        Assert.assertArrayEquals(new String[] { "bleh", "ert" },
                new TreeSet<>(auth.extractCredentials(null, newThreadContext("cn=abc,l=ert,L=bleh,st=zui,c=qwe")).getBackendRoles())
    ...
    public void testDNSpecials() throws Exception {
        ...
        final Settings settings = Settings.builder()
                ...
                .put("searchguard.cert.oid", "1.2.3.4.5.6").build();
        ...
        Settings tcSettings = Settings.builder()
                ...
                .put(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_TYPE, "PKCS12").build();
    ...
    public void testMultiget() throws Exception {
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            tc.index(new IndexRequest("mindex1").id("1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":1}", XContentType.JSON))
                    .actionGet();
            tc.index(new IndexRequest("mindex2").id("2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":2}", XContentType.JSON))
                    .actionGet();
        }

        //sg_multiget -> picard

        String mgetBody = "{" + "\"docs\" : [" + "{" + "\"_index\" : \"mindex1\"," + "\"_id\" : \"1\"" + " }," + " {" + "\"_index\" : \"mindex2\","
                + " \"_id\" : \"2\"" + "}" + "]" + "}";
    ...
    public void testRestImpersonation() throws Exception {
        ...
        resp = rh.executeGetRequest("/_searchguard/authinfo", new BasicHeader("sg_impersonate_as", "userwhonotexists"),
    ...
    public void testSingle() throws Exception {
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            tc.index(new IndexRequest("shakespeare").id("1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":1}", XContentType.JSON))
                    .actionGet();
    ...
    public void testXff() throws Exception {
        ...
        RestHelper rh = nonSslRestHelper();
        HttpResponse resc = rh.executeGetRequest("_searchguard/authinfo", new BasicHeader("x-forwarded-for", "10.0.0.7"),
    ...
    public void testRegexExcludes() throws Exception {
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            ...
            tc.index(new IndexRequest("alsonotallowed").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"alsonotallowed\":1}", XContentType.JSON))
                    .actionGet();
        }
    ...
    public void testUpdate() throws Exception {
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            tc.index(new IndexRequest("indexc").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":1}", XContentType.JSON))
                    .actionGet();
        }
    ...
    public void testSgIndexSecurity() throws Exception {
        ...
        res = rh.executePutRequest("*earc*gua*/_mapping?pretty", "{\"properties\": {\"name\":{\"type\":\"text\"}}}",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest("*/_mapping?pretty", "{\"properties\": {\"name\":{\"type\":\"text\"}}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
        res = rh.executePutRequest("_all/_mapping?pretty", "{\"properties\": {\"name\":{\"type\":\"text\"}}}",
                encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
        res = rh.executePostRequest("searchguard/_close", "", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executeDeleteRequest("searchguard", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executeDeleteRequest("_all", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
        res = rh.executePutRequest("*/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest("_settings", "{\"index\" : {\"number_of_replicas\" : 2}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest("_all/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest("searchguard/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
        res = rh.executePutRequest("searchgu*/_settings", "{\"index\" : {\"number_of_replicas\" : 2}}", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_FORBIDDEN, res.getStatusCode());
    ...
    public void testSgIndexSecurityWithSgIndexExcluded() throws Exception {
        ...
        res = rh.executeDeleteRequest("_all?ignore_unavailable=true", encodeBasicHeader("nagilum", "nagilum"));
        Assert.assertEquals(HttpStatus.SC_OK, res.getStatusCode());
    }
