...
public class TransportClientIntegrationTests extends SingleClusterTest {
    ...
    public void testTransportClient() throws Exception {
        ...
        try (Client tc = getInternalTransportClient(clusterInfo, tcSettings)) {
            ...
            IndexResponse ir = tc.index(
                    new IndexRequest("vulcan").id("s1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"secret\":true}", XContentType.JSON))
                    .actionGet();
            ...
            try {
                ...
                Assert.assertEquals("'CN=spock,OU=client,O=client,L=Test,C=DE' is not allowed to impersonate as transport user 'gkar'",
                        e.getMessage());
            }
    ...
    public void testTransportClientImpersonation() throws Exception {
        ...
        final Settings settings = Settings.builder()
                .putList("searchguard.authcz.impersonation_dn.CN=spock,OU=client,O=client,L=Test,C=DE", "worf", "nagilum").build();
        ...
        Settings tcSettings = Settings.builder()
                .put("searchguard.ssl.transport.keystore_filepath", FileHelper.getAbsoluteFilePathFromClassPath("spock-keystore.jks"))
                .put(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_ALIAS, "spock").put("path.home", ".")
                .put("request.headers.sg_impersonate_as", "worf").build();
    ...
    public void testTransportClientImpersonationWildcard() throws Exception {
        ...
        final Settings settings = Settings.builder().putList("searchguard.authcz.impersonation_dn.CN=spock,OU=client,O=client,L=Test,C=DE", "*")
                .build();
