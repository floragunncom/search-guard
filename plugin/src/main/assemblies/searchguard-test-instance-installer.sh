...
#
# Since you have full control over the CA and the certificates, they can
# be safely used also for production clusters.
...
ca:
   root:
      dn: CN=Example Com Inc. Root CA, OU=Example Com Inc. Root CA, O=Example Com Inc., DC=example, DC=com
      keysize: 2048
      validityDays: 3650
      pkPassword: none
      file: root-ca.pem
...
# node.key - the private key for the node certificate
# admin.pem - the admin certificate
# admin.key - the private key for the admin certificate
#
# ------------------------------------------------------------------------
...
#
# The files node.pem and node.key are used by your only node
# in the cluster to identify itself.
#
# The files admin.pem and admin.key can be used by administrators
...
cp $TLS_TOOL/certificates/node.key $SB_LC_NAME/config/node.key

cp $TLS_TOOL/certificates/admin.pem admin.pem
cp $TLS_TOOL/certificates/admin.key admin.key
...
tls:
  ...
  client_auth:
    ...
    private_key: "\${file:$BASE_DIR/admin.key}"
EOM
