...
public class DateMathTest extends AbstractDlsFlsTest {
    ...
    public void testSearch() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/%3Clogstash-%7Bnow%2Fd%7D%3E/_search?pretty", encodeBasicHeader("logstash", "password")))
                        .getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testFieldCaps() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/%3Clogstash-%7Bnow%2Fd%7D%3E/_field_caps?fields=*&pretty", encodeBasicHeader("logstash", "password")))
                        .getStatusCode());
        System.out.println(res.getBody());
        Assert.assertFalse(res.getBody().contains("ipaddr"));
        Assert.assertFalse(res.getBody().contains("message"));
    ...
    public void testSearchWc() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/logstash-*/_search?pretty", encodeBasicHeader("logstash", "password"))).getStatusCode());
        System.out.println(res.getBody());
        Assert.assertTrue(res.getBody().contains("\"value\" : 2,\n      \"relation"));
        Assert.assertTrue(res.getBody().contains("\"failed\" : 0"));
        Assert.assertFalse(res.getBody().contains("ipaddr"));
