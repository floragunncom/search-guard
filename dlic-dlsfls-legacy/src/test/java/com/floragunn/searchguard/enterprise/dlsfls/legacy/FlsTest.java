...
public class FlsTest extends AbstractDlsFlsTest {
    ...
    public void testFieldCapabilities() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_field_caps?fields=*&pretty", encodeBasicHeader("dept_manager_fls_reversed_fields", "password")))
                        .getStatusCode());
    ...
    public void testFlsGet() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_doc/0?realtime=true&pretty", encodeBasicHeader("dept_manager_fls_reversed_fields", "password")))
                        .getStatusCode());
        Assert.assertTrue(res.getBody().contains("\"found\" : true"));
        Assert.assertFalse(res.getBody().contains("cust1"));
        Assert.assertFalse(res.getBody().contains("cust2"));
