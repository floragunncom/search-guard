...
public class FieldMaskedTest extends AbstractDlsFlsTest {
    ...
    protected void populateData(Client tc) {
        ...
        tc.index(new IndexRequest("deals").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust1\"}, \"ip_source\": \"100.100.1.1\",\"ip_dest\": \"123.123.1.1\",\"amount\": 10}",
                XContentType.JSON)).actionGet();
        tc.index(new IndexRequest("deals").id("2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust2\"}, \"ip_source\": \"100.100.2.2\",\"ip_dest\": \"123.123.2.2\",\"amount\": 20}",
                XContentType.JSON)).actionGet();

        for (int i = 0; i < 30; i++) {
            tc.index(new IndexRequest("deals").id("a" + i).setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
    ...
    public void testMaskedAggregations() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executePostRequest("/deals/_search?pretty&size=0", query, encodeBasicHeader("user_masked", "password"))).getStatusCode());
    ...
    public void testMaskedAggregationsRace() throws Exception {
        ...
        Assert.assertTrue(res.getBody().contains("\"doc_count\" : 30"));
        Assert.assertTrue(res.getBody().contains("\"doc_count\" : 1"));
        Assert.assertFalse(res.getBody(), res.getBody().contains("100.100"));
    ...
    public void testMaskedSearch() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_search?pretty&size=100&q=100.100.1.1", encodeBasicHeader("admin", "admin"))).getStatusCode());
        Assert.assertTrue(res.getBody().contains("100.100.1.1"));
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_search?pretty&size=100&q=100.100.1.1", encodeBasicHeader("user_masked", "password")))
                        .getStatusCode());
    ...
    public void testMaskedSearchDCAggLocalHash() throws Exception {
        ...
        for (int i = 0; i < 15; i++) {
            Assert.assertEquals(HttpStatus.SC_OK,
                    (res = rh.executePostRequest("/deals/_search?pretty&size=0", agg, encodeBasicHeader("user_masked", "password"))).getStatusCode());
        ...
        System.out
                .println(rh.executeGetRequest("/deals/_stats/request_cache,query_cache?human&pretty", encodeBasicHeader("admin", "admin")).getBody());
