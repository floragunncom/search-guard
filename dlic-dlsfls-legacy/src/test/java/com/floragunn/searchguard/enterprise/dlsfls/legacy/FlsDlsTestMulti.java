...
public class FlsDlsTestMulti extends AbstractDlsFlsTest {
    ...
    protected void populateData(Client tc) {
        ...
        tc.index(new IndexRequest("deals").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE)
                .source("{\"customer\": {\"name\":\"cust1\"}, \"zip\": \"12345\",\"secret\": \"tellnoone\",\"amount\": 10}", XContentType.JSON))
                .actionGet();
    ...
    public void testDlsAggregations() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executePostRequest("/deals/_search?pretty", query, encodeBasicHeader("dept_manager_multi", "password"))).getStatusCode());
    ...
    public void testDlsFls() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_search?pretty&size=0", encodeBasicHeader("dept_manager_multi", "password"))).getStatusCode());
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executePostRequest("_msearch?pretty", msearchBody, encodeBasicHeader("dept_manager_multi", "password"))).getStatusCode());
        Assert.assertTrue(res.getBody(), res.getBody().contains("\"value\" : 3,\n          \"relation"));
        ...
        String mgetBody = "{" + "\"docs\" : [" + "{" + "\"_index\" : \"deals\"," + "\"_id\" : \"0\"" + " }," + " {" + "\"_index\" : \"deals\","
                + " \"_id\" : \"1\"" + "}," + " {" + "\"_index\" : \"deals\"," + " \"_id\" : \"2\"" + "}," + " {" + "\"_index\" : \"deals\","
                + " \"_id\" : \"3\"" + "}" + "]" + "}";
    ...
    public void testDlsSuggest() throws Exception {
        ...
        String query =
                ...
                "{" + "\"query\": {" + "\"range\" : {" + "\"amount\" : {" + "\"gte\" : 11," + "\"lte\" : 50000," + "\"boost\" : 1.0" + "}" + "}"
                        + "}," + "\"suggest\" : {\n" + "    \"thesuggestion\" : {\n" + "      \"text\" : \"cust\",\n" + "      \"term\" : {\n"
                        + "        \"field\" : \"customer.name\"\n" + "      }\n" + "    }\n" + "  }" + "}";
