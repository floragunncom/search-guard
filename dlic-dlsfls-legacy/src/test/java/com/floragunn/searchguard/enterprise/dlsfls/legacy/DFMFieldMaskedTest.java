...
public class DFMFieldMaskedTest extends AbstractDlsFlsTest {
    ...
    protected void populateData(Client tc) {
        ...
        tc.index(new IndexRequest("searchguard").id("config").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("config",
                FileHelper.readYamlContent("dlsfls_legacy/sg_config.yml"))).actionGet();
        tc.index(new IndexRequest("searchguard").setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("internalusers").source("internalusers",
                FileHelper.readYamlContent("dlsfls_legacy/sg_internal_users.yml"))).actionGet();
        tc.index(new IndexRequest("searchguard").id("roles").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("roles",
                FileHelper.readYamlContent("dlsfls_legacy/sg_roles.yml"))).actionGet();
        tc.index(new IndexRequest("searchguard").setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("rolesmapping").source("rolesmapping",
                FileHelper.readYamlContent("dlsfls_legacy/sg_roles_mapping.yml"))).actionGet();
        tc.index(new IndexRequest("searchguard").setRefreshPolicy(RefreshPolicy.IMMEDIATE).id("actiongroups").source("actiongroups",
                FileHelper.readYamlContent("dlsfls_legacy/sg_action_groups.yml"))).actionGet();

        tc.index(new IndexRequest("deals-0").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust1\"}, \"ip_source\": \"100.100.1.1\",\"ip_dest\": \"123.123.1.1\",\"amount\": 10}",
                XContentType.JSON)).actionGet();
        tc.index(new IndexRequest("deals-1").id("1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust2\"}, \"ip_source\": \"100.100.2.2\",\"ip_dest\": \"123.123.2.2\",\"amount\": 20}",
                XContentType.JSON)).actionGet();
        tc.index(new IndexRequest("deals-2").id("2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust3\"}, \"ip_source\": \"100.100.2.3\",\"ip_dest\": \"123.123.3.2\",\"amount\": 30}",
                XContentType.JSON)).actionGet();

        tc.index(new IndexRequest("deals-outdated-1").id("1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust4\"}, \"ip_source\": \"100.100.32.1\",\"ip_dest\": \"123.123.4.2\",\"amount\": 100}",
                XContentType.JSON)).actionGet();

        tc.index(new IndexRequest("deals-outdated-2").id("2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust5\"}, \"ip_source\": \"100.100.3.2\",\"ip_dest\": \"123.123.5.2\",\"amount\": 200}",
                XContentType.JSON)).actionGet();

        tc.index(new IndexRequest("deals-outdated-3").id("3").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(
                "{\"customer\": {\"name\":\"cust6\"}, \"ip_source\": \"100.100.3.3\",\"ip_dest\": \"123.123.6.2\",\"amount\": 300}",
                XContentType.JSON)).actionGet();

        tc.index(new IndexRequest("index1-1").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE)
                .source("{\"field1\": 1, \"field2\": \"value-2-1\", \"field3\": \"value-3-1\", \"field4\": \"value-4-1\" }", XContentType.JSON))
                .actionGet();

        tc.index(new IndexRequest("index1-2").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE)
                .source("{\"field1\": 2, \"field2\": \"value-2-2\", \"field3\": \"value-3-2\", \"field4\": \"value-4-2\" }", XContentType.JSON))
                .actionGet();

        tc.index(new IndexRequest("index1-3").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE)
                .source("{\"field1\": 3, \"field2\": \"value-2-3\", \"field3\": \"value-3-3\", \"field4\": \"value-4-3\" }", XContentType.JSON))
                .actionGet();

        tc.index(new IndexRequest("index1-4").id("0").setRefreshPolicy(RefreshPolicy.IMMEDIATE)
                .source("{\"field1\": 4, \"field2\": \"value-2-4\", \"field3\": \"value-3-4\", \"field4\": \"value-4-4\" }", XContentType.JSON))
                .actionGet();
    ...
    public void testMaskedSearch() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals-outdated-*/_search?pretty", encodeBasicHeader("admin", "admin"))).getStatusCode());
        System.out.println(res.getBody());

        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals-outdated-*/_search?pretty", encodeBasicHeader("dfm_user", "password"))).getStatusCode());
        System.out.println(res.getBody());

        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals-*/_search?pretty", encodeBasicHeader("dfm_user", "password"))).getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testDFMUnrestrictedUser() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/index1-*/_search?pretty", encodeBasicHeader("admin", "admin"))).getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testDFMRestrictedUser() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/_searchguard/authinfo?pretty", encodeBasicHeader("dfm_restricted_role", "password"))).getStatusCode());
        System.out.println(res.getBody());

        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/index1-*/_search?pretty", encodeBasicHeader("dfm_restricted_role", "password"))).getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testDFMRestrictedAndUnrestrictedAllIndices() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("/index1-*/_search?pretty",
                encodeBasicHeader("dfm_restricted_and_unrestricted_all_indices_role", "password"))).getStatusCode());
        System.out.println(res.getBody());
    ...
    public void testDFMRestrictedAndUnrestrictedOneIndex() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("/_searchguard/authinfo?pretty",
                encodeBasicHeader("dfm_restricted_and_unrestricted_one_index_role", "password"))).getStatusCode());
        System.out.println(res.getBody());
