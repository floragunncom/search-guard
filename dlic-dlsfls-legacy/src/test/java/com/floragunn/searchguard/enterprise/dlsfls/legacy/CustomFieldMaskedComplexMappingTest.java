...
public class CustomFieldMaskedComplexMappingTest extends AbstractDlsFlsTest {
    ...
    protected void populateData(Client tc) {
        ...
        try {
            tc.admin().indices().create(
                    new CreateIndexRequest("logs").mapping("_doc", FileHelper.loadFile("dlsfls_legacy/masked_field_mapping.json"), XContentType.JSON))
                    .actionGet();
    ...
    public void testComplexMappingSearch() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Assert.assertEquals(HttpStatus.SC_OK,
                    (res = rh.executeGetRequest("/logs/_search?pretty&size=100", encodeBasicHeader("user_masked_nowc", "password"))).getStatusCode());
