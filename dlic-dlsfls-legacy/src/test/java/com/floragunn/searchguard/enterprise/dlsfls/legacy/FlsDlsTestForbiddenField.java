...
public class FlsDlsTestForbiddenField extends AbstractDlsFlsTest {
    ...
    public void testDlsAggregations() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executePostRequest("/deals/_search?pretty", query, encodeBasicHeader("dept_manager_fls_dls", "password"))).getStatusCode());
    ...
    public void testDls() throws Exception {
        ...
        HttpResponse res;
        Assert.assertEquals(HttpStatus.SC_OK,
                (res = rh.executeGetRequest("/deals/_search?pretty&size=0", encodeBasicHeader("dept_manager_fls_dls", "password"))).getStatusCode());
        Assert.assertTrue(res.getBody().contains("\"value\" : 0,\n      \"relation"));
        Assert.assertTrue(res.getBody().contains("\"failed\" : 0"));
