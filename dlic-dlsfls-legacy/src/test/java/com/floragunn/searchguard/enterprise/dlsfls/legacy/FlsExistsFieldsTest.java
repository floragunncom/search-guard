...
public class FlsExistsFieldsTest extends AbstractDlsFlsTest {
    ...
    protected void populateData(Client tc) {
        ...
        tc.admin().indices().create(new CreateIndexRequest("data").mapping("_doc", "@timestamp", "type=date", "host", "type=text,norms=false",
                "response", "type=text,norms=false", "non-existing", "type=text,norms=false")).actionGet();
        ...
        for (int i = 0; i < 1; i++) {
            ...
            tc.index(new IndexRequest("data").id("b-missing1-" + i).setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(doc, XContentType.JSON))
                    .actionGet();
        }
        ...
        for (int i = 0; i < 1; i++) {
            ...
            tc.index(new IndexRequest("data").id("c-missing2-" + i).setRefreshPolicy(RefreshPolicy.IMMEDIATE).source(doc, XContentType.JSON))
                    .actionGet();
        }
