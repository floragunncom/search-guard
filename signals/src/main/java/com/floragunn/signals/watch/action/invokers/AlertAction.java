...
public class AlertAction extends ActionInvoker {
    ...
    protected final boolean acknowledgable;
    ...
    public AlertAction(String name, ActionHandler handler, DurationExpression throttlePeriod, SeverityLevel.Set severityLevels, List<Check> checks,
            InlinePainlessScript<SignalsObjectFunctionScript.Factory> foreach, Integer foreachLimit, boolean acknowledgable) {
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        if (!acknowledgable) {
            builder.field("acknowledgable", false);            
        }
    ...
    public static AlertAction create(WatchInitializationService watchInitService, DocNode jsonObject, SeverityMapping severityMapping)
        ...
        acknowledgable = vJsonNode.get("acknowledgable").withDefault(true).asBoolean();

        vJsonNode.checkForUnusedAttributes();

        validationErrors.throwExceptionForPresentErrors();

        return new AlertAction(name, handler, throttlePeriod, severityLevels, checks, foreach, foreachLimit, acknowledgable);
