...
public class GenericWatchAckTests extends AbstractGenericWatchTest {
    ...
    public void shouldAckOnlyOneWatchInstance() throws Exception {
        ...
        try(GenericRestClient restClient = cluster.getRestClient(USER_ADMIN).trackResources();
            ...
            // acked action state should not lose attributes instance_id and parent_generic_watch_id
            assertThat(body, containsValue("$.instance_id", instanceId2));
    ...
    public void shouldUnAckOnlyOneWatchInstance() throws Exception {
        ...
        try(GenericRestClient restClient = cluster.getRestClient(USER_ADMIN).trackResources();
            ...
            // unacked action state should not lose attributes instance_id and parent_generic_watch_id
            assertThat(body, containsValue("$.instance_id", instanceId2));
            assertThat(body, containsValue("$.parent_generic_watch_id", watchId));
        }
    }
