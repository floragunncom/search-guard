...
public class SettingsBasedSSLConfigurator {
    ...
    public SSLConfig buildSSLConfig() throws SSLConfigException {
        ...
        return new SSLConfig(sslContext, getSupportedProtocols(), getSupportedCipherSuites(), getHostnameVerifier(), isHostnameVerificationEnabled(),
                isTrustAllEnabled(), isStartTlsEnabled(), this.effectiveTruststore, this.effectiveTruststoreAliases, this.effectiveKeystore,
                this.effectiveKeyPassword, this.effectiveKeyAlias, this.isClientCertAuthenticationEnabled());
    }
    ...
    private void initFromPem() throws SSLConfigException {
        ...
        try {
            trustCertificates = PemKeyReader
                    .loadCertificatesFromStream(PemKeyReader.resolveStream(settingsKeyPrefix + PEMTRUSTEDCAS_CONTENT, settings));
        } catch (Exception e) {
            throw new SSLConfigException("Error loading PEM from " + settingsKeyPrefix + PEMTRUSTEDCAS_CONTENT + " for " + this.clientName, e);
        }
        ...
        if (trustCertificates == null) {
            ...
            try {
                ...
                throw new SSLConfigException(
                        "Error loading PEM from " + path + " (" + settingsKeyPrefix + PEMTRUSTEDCAS_FILEPATH + ") for " + this.clientName, e);
            }
        }
        ...
        if (authenticationCertificate == null) {
            ...
            try {
                ...
                throw new SSLConfigException(
                        "Error loading PEM from " + path + " (" + settingsKeyPrefix + PEMCERT_FILEPATH + ") for " + this.clientName, e);
            }
        ...
        if (authenticationKey == null) {
            ...
            try {
                ...
                throw new SSLConfigException(
                        "Error loading PEM from " + path + " (" + settingsKeyPrefix + PEMKEY_FILEPATH + ") for " + this.clientName, e);
            }
        }
    ...
    private void initFromKeyStore() throws SSLConfigException {
        ...
        try {
            trustStore = PemKeyReader.loadKeyStore(
                    PemKeyReader.resolve(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_TRUSTSTORE_FILEPATH, settings, configPath,
                            !isTrustAllEnabled()),
            ...
            throw new SSLConfigException(
                    "Error loading trust store from " + settings.get(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_TRUSTSTORE_FILEPATH), e);
        }
        ...
        try {
            keyStore = PemKeyReader.loadKeyStore(
                    PemKeyReader.resolve(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_FILEPATH, settings, configPath, enableSslClientAuth),
            ...
            throw new SSLConfigException(
                    "Error loading key store from " + settings.get(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_FILEPATH), e);
        }
    ...
    public static class SSLConfig {
        ...
        public SSLConfig(SSLContext sslContext, String[] supportedProtocols, String[] supportedCipherSuites, HostnameVerifier hostnameVerifier,
                boolean hostnameVerificationEnabled, boolean trustAll, boolean startTlsEnabled, KeyStore effectiveTruststore,
                List<String> effectiveTruststoreAliases, KeyStore effectiveKeystore, char[] effectiveKeyPassword, String effectiveKeyAlias,
        ...
        public String toString() {
            return "SSLConfig [sslContext=" + sslContext + ", supportedProtocols=" + Arrays.toString(supportedProtocols) + ", supportedCipherSuites="
                    ...
                    + ", effectiveKeyAlias=" + effectiveKeyAlias + ", clientCertAuth=" + clientCertAuth + "]";
        }
