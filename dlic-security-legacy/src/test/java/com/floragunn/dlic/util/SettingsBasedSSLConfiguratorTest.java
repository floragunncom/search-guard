...
public class SettingsBasedSSLConfiguratorTest {
    ...
    public void testPemTrust() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/pem/truststore.jks", "sslConfigurator/pem/node1-keystore.jks", "secret",
                false)) {
            ...
            Settings settings = Settings.builder().put("prefix.pemtrustedcas_filepath", rootCaPemPath.getFileName().toString())
    ...
    public void testPemClientAuth() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/pem/truststore.jks", "sslConfigurator/pem/node1-keystore.jks", "secret", true)) {
            ...
            Settings settings = Settings.builder().put("prefix.pemtrustedcas_filepath", rootCaPemPath.getFileName().toString())
                    ...
                    .put("prefix.enable_ssl_client_auth", "true").put("prefix.pemcert_filepath", "kirk.pem").put("prefix.pemkey_filepath", "kirk.key")
                    .put("prefix.pemkey_password", "secret").build();
            Path configPath = rootCaPemPath.getParent();
    ...
    public void testPemClientAuthFailure() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/pem/truststore.jks", "sslConfigurator/pem/node1-keystore.jks", "secret", true)) {
            ...
            Settings settings = Settings.builder().put("prefix.pemtrustedcas_filepath", rootCaPemPath.getFileName().toString())
                    ...
                    .put("prefix.pemkey_filepath", "wrong-kirk.key").put("prefix.pemkey_password", "G0CVtComen4a").build();
            Path configPath = rootCaPemPath.getParent();
    ...
    public void testPemHostnameVerificationFailure() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/pem/truststore.jks", "sslConfigurator/pem/node-wrong-hostname-keystore.jks",
                "secret", false)) {
            Path rootCaPemPath = FileHelper.getAbsoluteFilePathFromClassPath("sslConfigurator/pem/root-ca.pem");

            Settings settings = Settings.builder().put("prefix.pemtrustedcas_filepath", rootCaPemPath.getFileName().toString())
                    .put("prefix.enable_ssl", "true").put("prefix.verify_hostnames", "true").put("path.home", rootCaPemPath.getParent().toString())
                    .build();
            Path configPath = rootCaPemPath.getParent();
    ...
    public void testPemHostnameVerificationOff() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/pem/truststore.jks", "sslConfigurator/pem/node-wrong-hostname-keystore.jks",
            ...
            Settings settings = Settings.builder().put("prefix.pemtrustedcas_filepath", rootCaPemPath.getFileName().toString())
                    .put("prefix.enable_ssl", "true").put("prefix.verify_hostnames", "false").put("path.home", rootCaPemPath.getParent().toString())
                    .build();
            Path configPath = rootCaPemPath.getParent();
    ...
    public void testJksTrust() throws Exception {
        ...
        try (TestServer testServer = new TestServer("sslConfigurator/jks/truststore.jks", "sslConfigurator/jks/node1-keystore.jks", "secret",
                false)) {
            Path rootCaJksPath = FileHelper.getAbsoluteFilePathFromClassPath("sslConfigurator/jks/truststore.jks");

            Settings settings = Settings.builder().put("searchguard.ssl.transport.truststore_filepath", rootCaJksPath.getFileName().toString())
