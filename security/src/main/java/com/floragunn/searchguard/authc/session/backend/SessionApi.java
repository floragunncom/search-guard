        ...
        private RestChannelConsumer handleGet(RestRequest request, NodeClient client) {
            return (RestChannel channel) -> {
                try {
                    ...
                    if (user != null) {
                        channel.sendResponse(new BytesRestResponse(RestStatus.OK, "application/json", DocNode.of("sso_logout_url", sessionService.getSsoLogoutUrl(user)).toJsonString()));
                    } else {
            ...
            return (RestChannel channel) -> {
                try {
                    ...
                    sessionService.authenticateAndCreateSession(requestBody, request, (response) -> {
                        channel.sendResponse(new BytesRestResponse(RestStatus.CREATED, "application/json", response.toJsonString()));
                    }, (authFailureAuthzResult) -> {
                        channel.sendResponse(new BytesRestResponse(authFailureAuthzResult.getRestStatus(), "application/json",
