...
public class SessionToken implements ToXContentObject, Writeable, Serializable {
    ...
    public static SessionToken parse(String id, JsonNode jsonNode) throws ConfigValidationException {
        ...
        if (vJsonNode.hasNonNull("base")) {
            try {
                base = SessionPrivileges.parse(vJsonNode.get("base"));
        ...
        Instant dynamicExpiry = vJsonNode.value(SessionToken.DYNAMIC_EXPIRES_AT, (v) -> Instant.ofEpochMilli(v.longValue()), null);
