...
public class InternalAuthTokenProviderTest {
    ...
    static final SgDynamicConfiguration<Role> dlsRolesConfig = TestSgConfig.Role.toActualRole(getContextWithMatchAllQueryParser(),
            new TestSgConfig.Role("dls_role").indexPermissions("*").dls("{\"match_all\":{}}").on("my_index*"));
    static final Function<User, ImmutableSet<String>> dlsRoleMapper = (user) -> ImmutableSet.of("dls_role");
    ...
    public void userAuthFromToken_withDlsRole() throws Exception {
        InternalAuthTokenProvider subject = new InternalAuthTokenProvider(dlsRoleMapper, emptyActionGroupSupplier, emptyTenantNameSupplier, actions,
                () -> dlsRolesConfig, getContextWithMatchAllQueryParser());
        subject.setSigningKey(SIGNING_KEY);

        AuthFromInternalAuthToken result = subject.userAuthFromToken(JWT_WITH_DLS_ROLE, "test_audience");
