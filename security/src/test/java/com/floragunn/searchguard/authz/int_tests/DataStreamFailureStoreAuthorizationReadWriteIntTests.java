...
/**
 * Integration tests for Search Guard authorization of read and write operations on data streams with failure stores enabled.
 *
 * <p>This is a parameterized test class that runs each test against a set of users with varying privilege configurations,
 * verifying that access is correctly granted or denied for operations such as document indexing, bulk inserts,
 * delete-by-query, data stream lifecycle management (create/delete), and alias management.
 *
 * <p>All data streams are configured with failure stores enabled ({@code failureStoreEnabled(true)}), which distinguishes
 * these tests from the base class. This ensures that authorization behavior is correctly enforced also in the presence
 * of failure store backing indices.
 *
 * <p>Based on {@link DataStreamAuthorizationReadWriteIntTests}, extended to operate with failure stores enabled on data streams.
 */
@RunWith(Parameterized.class)
@NotThreadSafe
public class DataStreamFailureStoreAuthorizationReadWriteIntTests {
    private static final Logger log = LogManager.getLogger(DataStreamFailureStoreAuthorizationReadWriteIntTests.class);

    static TestDataStream ds_ar1 = TestDataStream.name("ds_ar1").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_ar2 = TestDataStream.name("ds_ar2").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_aw1 = TestDataStream.name("ds_aw1").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_aw2 = TestDataStream.name("ds_aw2").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_br1 = TestDataStream.name("ds_br1").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_br2 = TestDataStream.name("ds_br2").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_bw1 = TestDataStream.name("ds_bw1").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    static TestDataStream ds_bw2 = TestDataStream.name("ds_bw2").documentCount(22).rolloverAfter(10).failureStoreEnabled(true).failureDocumentCount(2).build();
    ...
    static TestDataStream ds_hidden = TestDataStream.name("ds_hidden").documentCount(10).rolloverAfter(3).failureStoreEnabled(true).failureDocumentCount(2).seed(8).attr("prefix", "h").build();
    ...
    public void deleteByQuery_indexPattern() throws Exception {
        try (GenericRestClient restClient = cluster.getRestClient(user)) {
            ...
            HttpResponse httpResponse = restClient.postJson("/ds_aw*,ds_bw*/_delete_by_query?refresh=true&wait_for_completion=true&pretty",
    ...
    public void putDocument_bulk() throws Exception {
        try (GenericRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (user == LIMITED_USER_PERMISSIONS_ON_BACKING_INDICES) {
                ...
                assertThat(httpResponse, containsExactly(ds_aw1.dataOnly(), ds_bw1.dataOnly()).at("items[*].create[?(@.result == 'created')]._index")
    ...
    public DataStreamFailureStoreAuthorizationReadWriteIntTests(TestSgConfig.User user, String description) throws Exception {
        this.user = user;
    }
