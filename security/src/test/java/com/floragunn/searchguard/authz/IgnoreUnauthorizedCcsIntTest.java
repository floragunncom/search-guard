...
@RunWith(Suite.class)
@Suite.SuiteClasses({
        IgnoreUnauthorizedCcsIntTest.SkipUnavailableRemoteClusterEnabledTest.class,
        IgnoreUnauthorizedCcsIntTest.SkipUnavailableRemoteClusterDisabledTest.class,
})
public class IgnoreUnauthorizedCcsIntTest {
    ...
    public static class SkipUnavailableRemoteClusterEnabledTest {
        ...
        public void search_clusterWildcard() throws Exception {
            ...
            try (GenericRestClient restClient = cluster.getRestClient(LIMITED_USER_COORD_A)) {
                ...
                Assert.assertThat(httpResponse, json(nodeAt("_clusters.details.my_remote.status", equalTo("skipped"))));
                Assert.assertThat(httpResponse, json(nodeAt("_clusters.details.my_remote.failures", hasSize(1))));
                Assert.assertThat(httpResponse, json(nodeAt("_clusters.details.my_remote.failures[0].reason.reason", equalTo("Insufficient permissions"))));
            }
        }
        ...
        public void msearch_staticIndicesInURL() throws Exception {
            ...
            try (GenericRestClient restClient = cluster.getRestClient(LIMITED_USER_COORD_B)) {
                ...
                Assert.assertThat(httpResponse, json(distinctNodesAt("responses[*]._clusters.details.my_remote.failures", containsInAnyOrder(hasSize(1)))));
                Assert.assertThat(httpResponse, json(distinctNodesAt("responses[*]._clusters.details.my_remote.failures[0].reason.reason", containsInAnyOrder("Insufficient permissions"))));
            }
        }
    ...
    /**
     * Covers only cases where the results differ from those with the option to skip unavailable clusters enabled {@link SkipUnavailableRemoteClusterEnabledTest}
     */
    @RunWith(Parameterized.class)
    public static class SkipUnavailableRemoteClusterDisabledTest {
