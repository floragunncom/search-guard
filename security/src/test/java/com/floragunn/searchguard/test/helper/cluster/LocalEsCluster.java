...
public class LocalEsCluster {
    ...
    public void start() throws Exception {
        ...
        CompletableFuture<Void> nonMasterNodeFuture = startNodes(clusterConfiguration.getNonMasterNodeSettings(),
    ...
    public ClusterInfo waitForCluster(ClusterHealthStatus status, TimeValue timeout, int expectedNodeCount) throws IOException {
        ...
        try {
            ...
            final ClusterHealthResponse healthResponse = adminClient.cluster().prepareHealth().setWaitForStatus(status).setTimeout(timeout)
                    .setMasterNodeTimeout(timeout).setWaitForNodes("" + expectedNodeCount).execute()
                    .actionGet();
            ...
            final NodesInfoResponse nodesInfoResponse = adminClient.cluster().nodesInfo(new NodesInfoRequest())
                    .actionGet();

            final List<NodeInfo> nodes = nodesInfoResponse.getNodes();

            final List<NodeInfo> masterNodes = nodes.stream()
                    .filter(nodeInfo -> nodeInfo.getNode().getRoles().contains(DiscoveryNodeRole.MASTER_ROLE))
                    .collect(Collectors.toList());

            final List<NodeInfo> dataNodes = nodes.stream()
                    .filter(nodeInfo -> nodeInfo.getNode().getRoles().contains(DiscoveryNodeRole.DATA_ROLE)
                            && !nodeInfo.getNode().getRoles().contains(DiscoveryNodeRole.MASTER_ROLE))
                    .collect(Collectors.toList());
            ...
            Predicate<NodeInfo> hasHttpAddressPredicate = nodeInfo -> nodeInfo.getInfo(HttpInfo.class) != null && nodeInfo.getInfo(HttpInfo.class).address() != null;
    ...
    public class Node {
        ...
        public String toString() {
            ...
            return String.format("%s %s [%d, %d]", nodeName, state, transportPort, httpPort);
        }
