...
public class DlsIntTest {
    ...
    public static LocalCluster.Embedded cluster = new LocalCluster.Builder().sslEnabled().enterpriseModulesEnabled().authc(AUTHC).dlsFls(DLSFLS)
            .users(ADMIN, DEPT_A_USER, DEPT_D_USER, DEPT_D_TERMS_LOOKUP_USER, LOGS_TWO_TERMX_A_USER).resources("dlsfls").embedded().build();
    ...
    public void testDlsWithMinDocCountZeroAggregations() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            client.index(new IndexRequest("logs-2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("amount", i, "termX", "A", "timestamp",
                    "2022-01-06T09:05:00Z")).actionGet();
            client.index(new IndexRequest("logs-2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("amount", i, "termX", "B", "timestamp",
                    "2022-01-06T09:08:00Z")).actionGet();
            client.index(new IndexRequest("logs-2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("amount", i, "termX", "C", "timestamp",
                    "2022-01-06T09:09:00Z")).actionGet();
            client.index(new IndexRequest("logs-2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("amount", i, "termX", "D", "timestamp",
                    "2022-01-06T09:10:00Z")).actionGet();
        }
        client.index(new IndexRequest("logs-2").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("amount", 0, "termX", "E", "timestamp",
