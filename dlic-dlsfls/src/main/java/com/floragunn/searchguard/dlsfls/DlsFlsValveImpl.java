...
public class DlsFlsValveImpl implements DlsFlsRequestValve {
    ...
    public boolean invoke(User user, ImmutableSet<String> mappedRoles, String action, ActionRequest request, ActionListener<?> listener,
        ...
        try {
            ...
            throw new OpenSearchSecurityException("Error while evaluating DLS/FLS configuration", e);
        }
            ...
            if (searchRequest.source() != null && searchRequest.source().aggregations() != null) {
                for (AggregationBuilder factory : searchRequest.source().aggregations().getAggregatorFactories()) {
                    if (factory instanceof TermsAggregationBuilder && ((TermsAggregationBuilder) factory).minDocCount() == 0) {
                        listener.onFailure(new OpenSearchSecurityException("min_doc_count 0 is not supported when DLS is activated"));
                        return false;
                    }
                }
            }
