...
public class DlsTermsLookupTest2 extends AbstractTLQTest {
    ...
    public static void setupTestData() {
        try (Client client = cluster.getInternalClient()) {
            ...
            client.index(new IndexRequest("tlqdummy").id("101").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{ \"mykey\": \"101\", \"bu\": \"GGG\" }",
                    XContentType.JSON)).actionGet();
            client.index(new IndexRequest("tlqdummy").id("102").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{ \"mykey\": \"102\", \"bu\": \"GGG\" }",
                    XContentType.JSON)).actionGet();
            client.index(new IndexRequest("tlqdummy").id("103").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{ \"mykey\": \"103\", \"bu\": \"GGG\" }",
                    XContentType.JSON)).actionGet();
            client.index(new IndexRequest("tlqdummy").id("104").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{ \"mykey\": \"104\", \"bu\": \"GGG\" }",
                    XContentType.JSON)).actionGet();
            client.index(new IndexRequest("tlqdummy").id("105").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{ \"mykey\": \"105\", \"bu\": \"GGG\" }",
                    XContentType.JSON)).actionGet();
    ...
    public void testSimpleSearch_AccessCodes_1337_42() throws Exception {
        ...
        try (RestHighLevelClient client = cluster.getRestHighLevelClient("tlq_1337", "password")) {                        
            ...
            Set<SearchHit> tlqdummyHits =  Arrays.asList(searchResponse.getHits().getHits()).stream().filter((h) -> h.getIndex().equals("tlqdummy")).collect(Collectors.toSet());
            ...
            Set<SearchHit> tlqdocumentHits =  Arrays.asList(searchResponse.getHits().getHits()).stream().filter((h) -> h.getIndex().equals("tlqdocuments")).collect(Collectors.toSet());
            ...
            Set<SearchHit> userAccessCodesHits =  Arrays.asList(searchResponse.getHits().getHits()).stream().filter((h) -> h.getIndex().equals("user_access_codes")).collect(Collectors.toSet());
