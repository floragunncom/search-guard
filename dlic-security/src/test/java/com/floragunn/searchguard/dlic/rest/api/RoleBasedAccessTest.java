...
public class RoleBasedAccessTest extends AbstractRestApiUnitTest {
    ...
    public void testActionGroupsApi() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        Assert.assertEquals(new SgJsonNode(DefaultObjectMapper.readTree(response.getBody()))
                .getDotted("sg_role_starfleet_captains.cluster_permissions").get(0).asString(), "cluster:monitor*");
        ...
        response = rh.executeGetRequest("/_searchguard/api/roles/sg_role_starfleet_captains", encodeBasicHeader("worf", "worf"));
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        Assert.assertEquals(new SgJsonNode(DefaultObjectMapper.readTree(response.getBody())).getDotted("sg_role_starfleet_captains.index_permissions")
                .get(0).get("allowed_actions").get(0).asString(), "blafasel");
