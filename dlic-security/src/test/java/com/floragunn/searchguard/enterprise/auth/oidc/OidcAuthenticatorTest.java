...
public class OidcAuthenticatorTest {
	...
	public static void setUp() throws Exception {
		...
		basicAuthenticatorSettings = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "client_id", "Der Klient", "client_secret", "Das Geheimnis",
	...
	public void basicTest() throws Exception {
		...
		authMethod = authenticator.activateFrontendConfig(authMethod,
				new GetFrontendConfigAction.Request(null, null, FRONTEND_BASE_URL));

		String ssoResponse = mockIdpServer.handleSsoGetRequestURI(authMethod.getSsoLocation(),
				TestJwts.MC_COY_SIGNED_OCT_1);

		Map<String, Object> request = ImmutableMap.of("sso_result", ssoResponse, "sso_context",
				authMethod.getSsoContext(), "frontend_base_url", FRONTEND_BASE_URL);
	...
	public void nextUrlTest() throws Exception {
		...
		Map<String, Object> request = ImmutableMap.of("sso_result", ssoResponse, "sso_context",
				authMethod.getSsoContext(), "frontend_base_url", FRONTEND_BASE_URL, "next_url", "http://redirect");
	...
	public void proxyTest() throws Exception {
		try (MockIpdServer proxyOnlyMockIdpServer = MockIpdServer.start(TestJwk.Jwks.ALL)
			...
			Map<String, Object> config = DocNode.of("idp.openid_configuration_url",
					mockIdpServer.getDiscoverUri().toString(), "proxy.host", "127.0.0.8", "proxy.port",
					httpProxy.getPort(), "proxy.scheme", "http", "client_id", "x", "client_secret", "x",
					"user_mapping.roles", "roles");

			OidcAuthenticator authenticator = new OidcAuthenticator(config, testContext);
			ActivatedFrontendConfig.AuthMethod authMethod = new ActivatedFrontendConfig.AuthMethod("oidc", "OIDC",
					null);
			authMethod = authenticator.activateFrontendConfig(authMethod,
					new GetFrontendConfigAction.Request(null, null, FRONTEND_BASE_URL));
	...
	public void testRoles() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.roles", TestJwts.ROLES_CLAIM, "client_id",
				"Der Klient", "client_secret", "Das Geheimnis");
	...
	public void testRolesJsonPath() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.roles", "$." + TestJwts.ROLES_CLAIM,
	...
	public void testRolesCollectionJsonPath() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.roles", "$." + TestJwts.ROLES_CLAIM,
				"user_mapping.subject", "$.sub", "client_id", "Der Klient", "client_secret", "Das Geheimnis");
		...
		String ssoResponse = mockIdpServer.handleSsoGetRequestURI(authMethod.getSsoLocation(), createSigned(
				create(MCCOY_SUBJECT, TEST_AUDIENCE, ROLES_CLAIM, Arrays.asList("role 1", "role 2", "role 3, role 4")),
				TestJwk.OCT_1));
		...
		Assert.assertNotNull(authCredentials);
		Assert.assertEquals(TestJwts.MCCOY_SUBJECT, authCredentials.getUsername());
		Assert.assertThat(authCredentials.getBackendRoles(),
				CoreMatchers.hasItems("role 1", "role 2", "role 3", "role 4"));
	}
	...
	public void testInvalidSubjectJsonPath() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.roles", "$." + TestJwts.ROLES_CLAIM,
				"user_mapping.subject", "$.subasd", "client_id", "Der Klient", "client_secret", "Das Geheimnis");
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains("The configured JSON Path $['subasd'] could not be found in the JWT"));
		}
	}
	...
	public void testInvalidRolesJsonPath() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.roles", "$.asd", "user_mapping.subject",
				"$.sub", "client_id", "Der Klient", "client_secret", "Das Geheimnis");
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains("The roles JSON path was not found in the Id token claims"));
		}
	}
	...
	public void testExp() throws Exception {
		...
		String ssoResponse = mockIdpServer.handleSsoGetRequestURI(authMethod.getSsoLocation(),
				TestJwts.MC_COY_EXPIRED_SIGNED_OCT_1);
	...
	public void testPeculiarJsonEscaping() throws Exception {
		...
		String ssoResponse = mockIdpServer.handleSsoGetRequestURI(authMethod.getSsoLocation(),
				TestJwts.PeculiarEscaping.MC_COY_SIGNED_RSA_1);
	...
	public void testSubjectPattern() throws Exception {
		Map<String, Object> config = ImmutableMap.of("idp.openid_configuration_url",
				mockIdpServer.getDiscoverUri().toString(), "user_mapping.subject_pattern", "^(.)(?:.*)$", "client_id",
