    ...
    private Map<Endpoint, List<Method>> parseDisabledEndpoints(Settings settings) {
        ...
        for (Entry<String, Object> value : disabledEndpointsSettings.entrySet()) {
            ...
            if (!(value.getValue() instanceof Collection)) {
                logger.error("Disabled HTTP methods of endpoint '{}' must be an array, actually is '{}', skipping.", endpointString,
            ...
            for (Object disabledMethodObj : (Collection) value.getValue()) {
                ...
                try {
                    ...
                    logger.error("Invalid HTTP method '{}' found in disabled HTTP methods of endpoint '{}', skipping.",
                            disabledMethodAsString.toUpperCase(), endpointString);
                    continue;
                }
        ...
        if (this.roleBasedAccessEnabled) {
            ...
            if (currentUserHasRestApiAccess(userRoles)) {
                ...
                if (!disabledMethodsForEndpoint.contains(request.method())) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Request method {} for user {} and endpoint {} not restricted, access allowed ", request.method(), user,
                                endpoint);
                    }
                    return null;
                }
                ...
                logger.info(
                        "User {} with Search Guard Roles {} does not have access to endpoint {} and method {}, checking admin TLS certificate now.",
                        user, userRoles, endpoint.name(), request.method());
                return "User " + user.getName() + " with Search Guard Roles " + userRoles + " does not have any access to endpoint " + endpoint.name()
