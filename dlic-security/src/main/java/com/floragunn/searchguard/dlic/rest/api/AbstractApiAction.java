...
public abstract class AbstractApiAction extends BaseRestHandler {
    ...
    protected void handleGet(final RestChannel channel, RestRequest request, Client client, final DocNode content) throws IOException {
    ...
    protected void saveAnUpdateConfigs(final Client client, final RestRequest request, final CType<?> cType, SgDynamicConfiguration<?> configuration,
        ...
        try {
            client.index(ir.id(id).setRefreshPolicy(RefreshPolicy.IMMEDIATE).setIfSeqNo(configuration.getSeqNo())
                    .setIfPrimaryTerm(configuration.getPrimaryTerm()).source(id, XContentHelper.toXContent(configuration, XContentType.JSON, false)),
