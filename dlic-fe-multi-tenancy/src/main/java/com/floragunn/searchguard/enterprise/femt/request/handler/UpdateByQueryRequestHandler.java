...
public class UpdateByQueryRequestHandler extends RequestHandler<UpdateByQueryRequest> {
    ...
    public SyncAuthorizationFilter.Result handle(PrivilegesEvaluationContext context, String requestedTenant,
        ...
        try (ThreadContext.StoredContext storedContext = threadContext.newStoredContext()) {
            ...
            nodeClient.execute(UpdateByQueryAction.INSTANCE, scoped, new ActionListener<>() {
                ...
                public void onResponse(BulkByScrollResponse response) {
                    try {
                        ...
                        BulkByScrollResponse unscoped = updateByQueryMapper.toUnscopedBulkByScrollResponse(response);
                        @SuppressWarnings("unchecked")
                        ActionListener<BulkByScrollResponse> updateListener = (ActionListener<BulkByScrollResponse>) listener;
                        updateListener.onResponse(unscoped);
                    } catch (Exception e) {
                        log.error("An error occurred while handling bulk by scroll response", e);
                ...
                public void onFailure(Exception e) {
                    log.error("An error occurred while sending update by query request", e);
                    listener.onFailure(e);
                }
            });
