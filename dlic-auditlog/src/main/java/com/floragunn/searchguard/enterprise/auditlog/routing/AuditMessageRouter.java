...
public class AuditMessageRouter {
    ...
    private final void setupRoutes(Settings settings) {
        Map<String, Object> routesConfiguration = Utils
                .convertJsonToxToStructuredMap(settings.getAsSettings(ConfigConstants.SEARCHGUARD_AUDIT_CONFIG_ROUTES));
        if (!routesConfiguration.isEmpty()) {
            ...
            for (Entry<String, Object> routesEntry : routesConfiguration.entrySet()) {
                ...
                try {
                    ...
                    List<AuditLogSink> sinksForCategory = createSinksForCategory(category,
                            settings.getAsSettings(ConfigConstants.SEARCHGUARD_AUDIT_CONFIG_ROUTES + "." + categoryName));
