...
public class AsyncStoragePool {
    ...
    public AsyncStoragePool(final Settings settings) {
        ...
        this.threadPoolMaxQueueLen = settings.getAsInt(ConfigConstants.SEARCHGUARD_AUDIT_THREADPOOL_MAX_QUEUE_LEN, DEFAULT_THREAD_POOL_MAX_QUEUE_LEN)
                .intValue();
    ...
    public void submit(AuditMessage message, AuditLogSink sink) {
        try {
            ...
            log.error("Could not submit audit message {} to thread pool for delegate '{}' due to '{}'", message, sink.getClass().getSimpleName(),
                    ex.getMessage());
