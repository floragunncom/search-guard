...
public class AuditLogConfig implements LicenseChangeListener {
    ...
    public static final StaticSettings.Attribute<List<String>> COMPLIANCE_HISTORY_READ_WATCHED_FIELDS = StaticSettings.Attribute
            .define("searchguard.compliance.history.read.watched_fields").asListOfStrings();
    ...
    public AuditLogConfig(Environment environment, ConfigurationRepository configRepository) {
        ...
        if ("internal_elasticsearch".equalsIgnoreCase(type)) {
            final String index = settings.get(ConfigConstants.SEARCHGUARD_AUDIT_CONFIG_DEFAULT_PREFIX + ConfigConstants.SEARCHGUARD_AUDIT_ES_INDEX,
        ...
        cache = CacheBuilder.newBuilder().maximumSize(1000).build(new CacheLoader<String, Set<String>>() {
            @Override
