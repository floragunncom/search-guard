...
public class IgnoreRequestBodiesTest {
    ...
    @Test
    public void testNonSessionEndpointBodyIsLogged() throws Exception {
        String jsonBody = "{\"query\":{\"match_all\":{}}}";
        UserInformation userInformation = UserInformation.forName("testuser");
        try (AbstractAuditLog al = new AuditLogImpl(settingsBuilder.build(), null, null, newThreadPool(), null, cs, configurationRepository)) {
            TestAuditlogImpl.clear();
            al.logSucceededLogin(userInformation, false, userInformation, new MockRestRequest("/some/other/path", jsonBody));
            assertEquals(1, TestAuditlogImpl.messages.size());
            assertTrue(TestAuditlogImpl.messages.get(0).getAsMap().containsKey(REQUEST_BODY));
        }
    }
    ...
    @Test
    public void testSessionEndpointBodyExcludedWhenExplicitlyConfigured() throws Exception {
        String jsonBody = "{\"password\":\"secret\"}";
        UserInformation userInformation = UserInformation.forName("testuser");
        Settings settings = settingsBuilder
                .putList(ConfigConstants.SEARCHGUARD_AUDIT_IGNORE_REQUEST_BODIES, "/_searchguard/auth/session")
                .build();
        try (AbstractAuditLog al = new AuditLogImpl(settings, null, null, newThreadPool(), null, cs, configurationRepository)) {
            TestAuditlogImpl.clear();
            al.logSucceededLogin(userInformation, false, userInformation, new MockRestRequest("/_searchguard/auth/session", jsonBody));
