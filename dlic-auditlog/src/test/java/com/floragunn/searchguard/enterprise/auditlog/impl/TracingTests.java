...
public class TracingTests extends SingleClusterTest {
    ...
    public void testHTTPTrace() throws Exception {
        ...
        final Settings settings = Settings.builder().put(ConfigConstants.SEARCHGUARD_AUDIT_TYPE_DEFAULT, "debug")
                .put(ConfigConstants.SEARCHGUARD_COMPLIANCE_HISTORY_READ_WATCHED_FIELDS, "*")
                .put(ConfigConstants.SEARCHGUARD_COMPLIANCE_HISTORY_WRITE_WATCHED_INDICES, "*").put("searchguard.audit.resolve_bulk_requests", true)
                .put("searchguard.audit.config.log4j.logger_name", "sg_action_trace").put("searchguard.audit.config.log4j.level", "TRACE").build();
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            ...
            for (int i = 0; i < 50; i++) {
                tc.index(new IndexRequest("a").id(i + "").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":" + i + "}",
                        XContentType.JSON)).actionGet();
                tc.index(new IndexRequest("c").id(i + "").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("{\"content\":" + i + "}",
                        XContentType.JSON)).actionGet();
            }
        }
        ...
        String bulkBody = "{ \"index\" : { \"_index\" : \"test\", \"_type\" : \"type1\", \"_id\" : \"1\" } }" + System.lineSeparator()
        ...
        System.out.println("############ _bulk");
        bulkBody = "{ \"index\" : { \"_index\" : \"test\", \"_type\" : \"type1\", \"_id\" : \"1\" } }" + System.lineSeparator()
                + "{ \"field1\" : \"value1\" }" + System.lineSeparator()
                + "{ \"index\" : { \"_index\" : \"test\", \"_type\" : \"type1\", \"_id\" : \"2\" } }" + System.lineSeparator()
                + "{ \"field2\" : \"value2\" }" + System.lineSeparator()
                + "{ \"delete\" : { \"_index\" : \"test\", \"_type\" : \"type1\", \"_id\" : \"2\" } }" + System.lineSeparator();
        ...
        String msearchBody = "{\"index\":\"a\", \"type\":\"b\", \"ignore_unavailable\": true}" + System.lineSeparator()
        ...
        String mgetBody = "{" + "\"docs\" : [" + "{" + "\"_index\" : \"a\"," + "\"_type\" : \"b\"," + "\"_id\" : \"1\"" + " }," + " {"
                + "\"_index\" : \"a\"," + " \"_type\" : \"b\"," + " \"_id\" : \"12\"" + "}," + " {" + "\"_index\" : \"a\"," + " \"_type\" : \"b\","
                + " \"_id\" : \"13\"" + "}," + " {" + "\"_index\" : \"a\"," + " \"_type\" : \"b\"," + " \"_id\" : \"14\"" + "}" + "]" + "}";
    ...
    public void testHTTPSingle() throws Exception {
        ...
        AsyncAssert.awaitAssert("_search is OK", () -> rh.executeGetRequest("_search", encodeBasicHeader("admin", "admin")).getStatusCode() == 200,
    ...
    public void testAdvancedMapping() throws Exception {
        Settings settings = Settings.builder().put(ConfigConstants.SEARCHGUARD_COMPLIANCE_HISTORY_READ_WATCHED_FIELDS, "*")
        ...
        try (Client tc = getPrivilegedInternalNodeClient()) {
            tc.admin().indices().create(new CreateIndexRequest("myindex1").mapping("_doc", FileHelper.loadFile("mapping1.json"), XContentType.JSON))
                    .actionGet();
            tc.admin().indices().create(new CreateIndexRequest("myindex2").mapping("_doc", FileHelper.loadFile("mapping2.json"), XContentType.JSON))
                    .actionGet();
            tc.admin().indices().create(new CreateIndexRequest("myindex3").mapping("_doc", FileHelper.loadFile("mapping3.json"), XContentType.JSON))
                    .actionGet();
            tc.admin().indices().create(new CreateIndexRequest("myindex4").mapping("_doc", FileHelper.loadFile("mapping4.json"), XContentType.JSON))
                    .actionGet();
        }
        ...
        System.out.println(rh.executeGetRequest(
                "myindex1/mytype1/1?pretty=true&_source=true&_source_include=*.id&_source_exclude=entities&stored_fields=tags,counter",
                encodeBasicHeader("admin", "admin")).getBody());
