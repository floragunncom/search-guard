...
public class UnparsedDocTest {
    ...
    @Test
    public void testEncodingConversion() throws DocParseException {
        UnparsedDoc<?> unparsedDoc = UnparsedDoc.from(UTF16_BE, DocType.JSON);
        Assert.assertEquals("你好", unparsedDoc.parseAsDocNode().get("hello"));
        unparsedDoc = UnparsedDoc.from(UTF16_LE, DocType.JSON);
        Assert.assertEquals("你好", unparsedDoc.parseAsDocNode().get("hello"));
        unparsedDoc = UnparsedDoc.from(UTF32_BE, DocType.JSON);
        Assert.assertEquals("你好", unparsedDoc.parseAsDocNode().get("hello"));
        unparsedDoc = UnparsedDoc.from(UTF8, DocType.JSON);
        Assert.assertEquals("你好", unparsedDoc.parseAsDocNode().get("hello"));
    }

    @Test
    public void testDocTypeConversion() throws DocParseException {
        UnparsedDoc<?> unparsedDoc = UnparsedDoc.from("{\"a\": [1,2,3]}", DocType.JSON);
        Assert.assertEquals(Arrays.asList(1, 2, 3), UnparsedDoc.from(unparsedDoc.toYamlString(), DocType.YAML).parseAsDocNode().get("a"));
    }
}
