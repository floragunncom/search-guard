...
public class JsonPathPatchTest {
    @Test
    public void basicTest() {
        DocNode targetDocument = DocNode.of("a", "ao", "b", DocNode.of("b_x", "bo1", "b_z", "bo3"), "c", "co");
        JsonPathPatch jsonPathPatch = new JsonPathPatch(new JsonPathPatch.Operation(JsonPath.compile("b.b_x"), 44));
    ...
    public void parseTest() throws DocumentParseException, ConfigValidationException {
        DocNode targetDocument = DocNode.of("a", "ao", "b", DocNode.of("b_x", "bo1", "b_z", "bo3"), "c", "co");
        JsonPathPatch jsonPathPatch = new JsonPathPatch(new JsonPathPatch.Operation(JsonPath.compile("b.b_x"), 44));
        JsonPathPatch jsonPathPatch2 = new JsonPathPatch(DocNode.parse(Format.JSON).from(jsonPathPatch.toJsonString()));
