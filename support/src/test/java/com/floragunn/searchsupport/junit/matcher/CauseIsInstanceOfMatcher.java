...
class CauseIsInstanceOfMatcher extends TypeSafeDiagnosingMatcher<Throwable> {
    ...
    protected boolean matchesSafely(Throwable item, Description mismatchDescription) {
        ...
        if( ! causeType.isInstance(cause)) {
            mismatchDescription.appendText(" has not expected cause type ").appendValue(cause);
            return false;
        }
        return true;
    }
    ...
    public void describeTo(Description description) {
        description.appendText("Exception contain a cause of type ").appendValue(causeType);
    }
}
